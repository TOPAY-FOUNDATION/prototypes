...
export class Block {
  ...
  async mineBlock(difficulty = this.difficulty) {
    ...
    while (true) {
      ...
      if (this.hash.substring(0, difficulty) === target) {
        ...
        console.log(`[MINING] Block mined in ${miningTime}s! Hash: ${this.hash}`);
  ...
  async isValid() {
    ...
    if (this.hash !== calculatedHash) {
      console.log('[VALIDATION] Block hash is invalid');
      return false;
    }
    ...
    if (this.transactions.length > 0) {
      ...
      if (this.merkleRoot !== calculatedMerkleRoot) {
        console.log('[VALIDATION] Merkle root is invalid');
    ...
    for (const transaction of this.transactions) {
      if (!(await transaction.isValid())) {
        console.log('[VALIDATION] Invalid transaction found in block');
        return false;
      }
    }
    ...
    if (this.hash.substring(0, this.difficulty) !== target) {
      console.log('[VALIDATION] Block does not meet difficulty requirement');
      return false;
    }
  ...
  async fragmentBlock() {
    ...
    if (blockData.length > 2048) { // Fragment if larger than 2KB
      console.log(`[FRAGMENT] Fragmenting block (${blockData.length} bytes)...`);
  ...
  static async reconstructFromFragments(fragments) {
    console.log(`[RECONSTRUCT] Reconstructing block from ${fragments.length} fragments...`);
    ...
    console.log(`[RECONSTRUCT] Block reconstructed successfully`);
    return block;
  }
